...
class TransactionDisplayCrudController extends CrudController
{
    ...
    public function setup()
    {
        Transaction::addGlobalScope('selectedType', function (Builder $builder) {
            $builder->where($builder->getQuery()->from . '.' . 'type', 'Display');
        });
        if(!backpack_user()->hasPermissionTo('Read Display Transaction')){
            $this->crud->denyAccess(['list']);
        }
        if(!backpack_user()->hasPermissionTo('Create Display Transaction')){
            $this->crud->denyAccess(['create']);
        }
        if(!backpack_user()->hasPermissionTo('Delete Display Transaction')){
            $this->crud->denyAccess(['delete']);
        }
        if(!backpack_user()->hasPermissionTo('Detail Display Transaction')){
        ...
        try {
            ...
            foreach ($products as $key => $item) {
                $product = Product::
                    ...
                    ->first();
                $totalPrice += $product->netto_price * $item['quantity'] - ($product->netto_price * $item['quantity'] * $item['discount_percentage'] / 100);
            }
            ...
            if(backpack_user()->hasPermissionTo('Create Payment Transaction')){
                ...
                return redirect(backpack_url('transaction-display') . '/' . $transaction->id . '/show');
            }
        } catch (\Exception $e) {
            DB::rollback();
